...
def evidence_for_video(model: ChannelRankVerifier, item: dict, device: torch.device, topk: int) -> dict[str, np.ndarray]:
    ...
    return {
        ...
        "subspace_residual": outputs["subspace_residual"][0, :length].cpu().numpy().astype(np.float32),
        "subspace_deviation": outputs["subspace_deviation"][0, :length].cpu().numpy().astype(np.float32),
        "subspace_excess": outputs["subspace_excess"][0, :length].cpu().numpy().astype(np.float32),
        "signed_text_residual": outputs["signed_text_residual"][0, :length].cpu().numpy().astype(np.float32),
        "semantic_excess": outputs["semantic_excess"][0, :length].cpu().numpy().astype(np.float32),
        ...
        "subspace_gate": outputs["subspace_gates"].cpu().numpy().astype(np.float32),
        "semantic_gate": outputs["semantic_gates"].cpu().numpy().astype(np.float32),
        ...
        "subspace_weight": outputs["subspace_weights"].cpu().numpy().astype(np.float32),
        ...
        "subspace_threshold": outputs["subspace_threshold"].cpu().numpy().astype(np.float32),
        "semantic_threshold": outputs["semantic_threshold"].cpu().numpy().astype(np.float32),
        ...
        "subspace_scale": outputs["subspace_scales"].cpu().numpy().astype(np.float32),
        "semantic_scale": outputs["semantic_scales"].cpu().numpy().astype(np.float32),
        "semantic_topk_circuit_index_by_class": outputs["semantic_topk_indices"][0, :length].cpu().numpy().astype(np.int64),
        "semantic_topk_contribution_by_class": outputs["semantic_topk_values"][0, :length].cpu().numpy().astype(np.float32),
