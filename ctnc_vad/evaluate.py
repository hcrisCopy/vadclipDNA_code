    ...
    try:
        ...
        try:
            return all(
                ...
                for name in (
                    "prob1", "prob2", "evidence", "sparse_evidence", "semantic_evidence", "memory_evidence",
...
def verifier_sequence(
    ...
    detail = {
        ...
        "normal_memory_anomaly": outputs["normal_memory_anomaly"][0, :length].detach().cpu().numpy().astype(np.float32),
...
def collect_predictions(
    ...
    output = {
        ...
        "sparse_evidence": [], "semantic_evidence": [], "memory_evidence": [],
    ...
    for item in tqdm(dataset, desc=progress, unit="video"):
        ...
        if target is not None and reuse_predictions and valid_prediction(target, length):
            ...
            result = {
                ...
                "memory_evidence": detail["normal_memory_anomaly"],
                "verified": verified,
...
def summarize_predictions(predictions: dict[str, list[np.ndarray]], labels: list[str], gt: np.ndarray, gtsegments: np.ndarray, gtlabels: np.ndarray, dataset: str) -> dict:
    ...
    return {
        ...
        "normal_memory_evidence_only": score_only_metrics(predictions["memory_evidence"], gt),
